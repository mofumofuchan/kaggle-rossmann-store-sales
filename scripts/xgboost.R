...
# Google trends
google_trends$week <- week(google_trends$Dates)
google_trends$year <- year(google_trends$Dates)
google_trends$Dates <- NULL
train <- merge(train, google_trends, by=c("week", "year"))
test <- merge(test, google_trends, by=c("week", "year"))
train$Trend <- as.numeric(train$Trend)
test$Trend <- as.numeric(test$Trend)

feature.names <- names(train)[c(1:4,7:23)]
cat("Feature Names\n")
feature.names
...
write_csv(submission, "../submissions/xgboost_with_trends.csv")
